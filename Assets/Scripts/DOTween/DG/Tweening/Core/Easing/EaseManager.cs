	...
	public static class EaseManager
	{
		...
		public static float Evaluate(Ease easeType, EaseFunction customEase, float time, float duration, float overshootOrAmplitude, float period)
		{
			...
			case Ease.InOutCirc:
				...
				{
					float num3 = time;
					return -0.5f * ((float)Math.Sqrt(1f - num3 * num3) - 1f);
				}
			...
			case Ease.InElastic:
			{
				...
				{
					num5 = period / ((float)Math.PI * 2f) * (float)Math.Asin(1f / overshootOrAmplitude);
				}
				return 0f - overshootOrAmplitude * (float)Math.Pow(2.0, 10f * (time -= 1f)) * (float)Math.Sin((time * duration - num5) * ((float)Math.PI * 2f) / period);
			}
			case Ease.OutElastic:
			{
				...
				{
					num4 = period / ((float)Math.PI * 2f) * (float)Math.Asin(1f / overshootOrAmplitude);
				}
				return overshootOrAmplitude * (float)Math.Pow(2.0, -10f * time) * (float)Math.Sin((time * duration - num4) * ((float)Math.PI * 2f) / period) + 1f;
			}
			...
			case Ease.InOutBack:
				...
				{
					float num = time;
					return 0.5f * (num * num * (((overshootOrAmplitude *= 1.525f) + 1f) * time - overshootOrAmplitude));
				}
		...
		public static EaseFunction ToEaseFunction(Ease ease)
		{
			...
			case Ease.InElastic:
				return delegate(float time, float duration, float overshootOrAmplitude, float period)
				{
					...
					return 0f - overshootOrAmplitude * (float)Math.Pow(2.0, 10f * (time -= 1f)) * (float)Math.Sin((time * duration - num4) * ((float)Math.PI * 2f) / period);
				};
			case Ease.OutElastic:
				return delegate(float time, float duration, float overshootOrAmplitude, float period)
				{
					...
					return overshootOrAmplitude * (float)Math.Pow(2.0, -10f * time) * (float)Math.Sin((time * duration - num5) * ((float)Math.PI * 2f) / period) + 1f;
				};
			case Ease.InOutElastic:
				return delegate(float time, float duration, float overshootOrAmplitude, float period)
				{
			...
			case Ease.InOutBack:
				return delegate(float time, float duration, float overshootOrAmplitude, float period)
				{
					...
					{
						float num2 = time;
						return 0.5f * (num2 * num2 * (((overshootOrAmplitude *= 1.525f) + 1f) * time - overshootOrAmplitude));
					}
					return 0.5f * ((time -= 2f) * time * (((overshootOrAmplitude *= 1.525f) + 1f) * time + overshootOrAmplitude) + 2f);
				};
			case Ease.InBounce:
