namespace CraftyVoxelEngine
{
	public class MessageUnknownMessage : DataSerializable
	{
		public int MESSAGE_ID;

		public int MESSAGE_SEQ;

		public int MESSAGE_STATUS;

		public int messageID;

		public int messageCount;

		public MessageUnknownMessage()
		{
			MESSAGE_ID = 5001;
			MESSAGE_SEQ = 0;
			MESSAGE_STATUS = 0;
		}

		public override void Deserialize(DataBuffer buffer)
		{
			MESSAGE_ID = buffer.ReadInt();
			MESSAGE_SEQ = buffer.ReadInt();
			MESSAGE_STATUS = buffer.ReadInt();
			messageID = buffer.ReadInt();
			messageCount = buffer.ReadInt();
		}

		public override void Serialize(DataBuffer buffer)
		{
			buffer.WriteInt(MESSAGE_ID);
			buffer.WriteInt(MESSAGE_SEQ);
			buffer.WriteInt(MESSAGE_STATUS);
			buffer.WriteInt(messageID);
			buffer.WriteInt(messageCount);
		}

		public override string ToString()
		{
			return string.Format("MESSAGE_ID: {0} MESSAGE_SEQ: {1} MESSAGE_STATUS: {2} messageID: {3} messageCount: {4}", MESSAGE_ID, MESSAGE_SEQ, MESSAGE_STATUS, messageID, messageCount);
		}
	}
}
