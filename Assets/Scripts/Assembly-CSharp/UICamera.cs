...
public class UICamera : MonoBehaviour
{
	...
	public void ProcessEvents()
	{
		...
		{
			float num = (string.IsNullOrEmpty("Mouse ScrollWheel") ? 0f : GetAxis("Mouse ScrollWheel"));
			if (num != 0f)
			{
				if (onScroll != null)
				{
	...
	public void ProcessOthers()
	{
		...
		if (useController && !ignoreControllerInput)
		{
			...
			{
				int direction = GetDirection("Vertical");
				if (direction != 0)
				{
					ShowTooltip(null);
			...
			{
				int direction2 = GetDirection("Horizontal");
				if (direction2 != 0)
				{
					ShowTooltip(null);
					currentScheme = ControlScheme.Controller;
	...
	private void ProcessRelease(bool isMouse, float drag)
	{
		...
		if (currentTouch.pressed != null)
		{
			...
			if (currentTouch.dragStarted)
			{
				Notify(currentTouch.dragged, "OnDropEnd", null);
			}
		}
		currentTouch.dragStarted = false;
