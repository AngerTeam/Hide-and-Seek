...
public class UIWidget : UIRect
{
	...
	public int width
	{
		...
		set
		{
			...
			if (mWidth == value || IsWidthFixed(keepAspectRatio))
			{
				return;
			}
	...
	public int height
	{
		...
		set
		{
			...
			if (mHeight == value || IsHeightFixed(keepAspectRatio))
			{
				return;
			}
	...
	public bool IsWidthFixed(AspectRatioSource aspect)
	{
		return aspect == AspectRatioSource.BasedOnWidth || aspect == AspectRatioSource.FitInParent || aspect == AspectRatioSource.EnvelopeParent;
	}

	public bool IsHeightFixed(AspectRatioSource aspect)
	{
		return aspect == AspectRatioSource.BasedOnHeight || aspect == AspectRatioSource.FitInParent || aspect == AspectRatioSource.EnvelopeParent;
	}
	...
	public void SetDimensions(int w, int h)
	{
		if (mWidth != w || mHeight != h)
		{
			mWidth = w;
			mHeight = h;
			AspectRationCalculation(ref w, ref h);
			mMoved = true;
			if (autoResizeBoxCollider)
			{
				ResizeCollider();
	...
	public void CheckLayer()
	{
		if (panel != null && panel.gameObject.layer != base.gameObject.layer)
		{
			...
			UnityEngine.Debug.LogWarning("panel:", panel.gameObject);
	...
	protected override void OnAnchor()
	{
		...
		int w = Mathf.FloorToInt(num2 - num + 0.5f);
		int h = Mathf.FloorToInt(num4 - num3 + 0.5f);
		AspectRationCalculation(ref w, ref h);
		if (w < minWidth)
		{
			w = minWidth;
	...
	private void AspectRationCalculation(ref int w, ref int h)
	{
		aspectRatio = Mathf.Max(0.001f, aspectRatio);
		switch (keepAspectRatio)
		{
		case AspectRatioSource.BasedOnWidth:
			h = Mathf.RoundToInt((float)w / aspectRatio);
			break;
		case AspectRatioSource.BasedOnHeight:
			w = Mathf.RoundToInt((float)h * aspectRatio);
			break;
		case AspectRatioSource.FitInParent:
		{
			float num2 = (float)w / (float)h;
			if (num2 < aspectRatio)
			{
				h = Mathf.RoundToInt((float)w / aspectRatio);
			}
		...
		case AspectRatioSource.EnvelopeParent:
		{
