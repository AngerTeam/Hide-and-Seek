	...
	public class ISD_LibHandler : MonoBehaviour
	{
		public static List<Lib> AvailableLibraries
		{
			get
			{
				...
				foreach (int value in Enum.GetValues(typeof(iOSLibrary)))
				{
					if (list2.Contains(((iOSLibrary)value).ToString()))
					{
						list.Add(new Lib((iOSLibrary)value));
					}
				}
				return list;
			}
		...
		public static object enumValueOf(string value)
		{
			...
			{
				if (stringValueOf((iOSLibrary)(int)Enum.Parse(typeFromHandle, value2)).Equals(value))
				{
